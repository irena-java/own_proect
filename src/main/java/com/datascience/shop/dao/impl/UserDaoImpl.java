...
public class UserDaoImpl implements UserDao {
    ...
    private static final String USER_FIELD =
            "name,user_role_id,client_inn,country_id,contact_info,password";
    ...
    private static final String INSERT_SQL =
            "INSERT INTO users (" + USER_FIELD + ") VALUES (?,?,?,?,?,?)";
    private static final String GET_COUNTRY_ID_BY_NAME =
            "SELECT id FROM countries WHERE country=?";
    private static final String GET_ROLE_ID_BY_NAME =
            "SELECT id FROM user_roles WHERE user_role=?";
    ...
    public Integer create(User user) throws DaoException {
        try (
            ...
            if (getCountryId(user.getCountry()) != -1) {
                ...
                throw new DaoException("Country " + user.getCountry() +
            ...
            if (user.getUserRole() == UserRole.ADMIN) {
                logger.warn("ATTENTION. CREATE NEW ADMIN." + user.toString());
            }

            logger.debug("Successfully create user " + user.getName() +
                    ", id = " + user.getId());
            return resultSet.getInt(1);
        } catch (SQLException e) {
            logger.error("Failed to create user " + user.toString() + e);
            throw new DaoException("Failed to create user " + user.toStringShort() + e);
        }
    }
    ...
    public int getCountryId(String country) throws DaoException {
        try (
                Connection connection = ControllerFactory.connectionPoolImpl.get();
                PreparedStatement preparedStatement =
                        connection.prepareStatement(GET_COUNTRY_ID_BY_NAME)) {
            ...
            logger.error("Failed getCountryId for country " + country + e);
            throw new DaoException("Failed getCountryId for country " + country + e);
        }
    }
    ...
    public int getRoleId(UserRole userRole) throws DaoException {
        try (
                Connection connection = ControllerFactory.connectionPoolImpl.get();
                PreparedStatement preparedStatement =
                        connection.prepareStatement(GET_ROLE_ID_BY_NAME)) {
            ...
            throw new DaoException("Failed getRoleId for userRole  " + userRole + e);
        }
    }
    ...
    public User findByUsername(String username) throws DaoException {
        try (
                Connection connection = ControllerFactory.connectionPoolImpl.get();
                PreparedStatement preparedStatement =
                        connection.prepareStatement(SELECT_BY_USER_NAME)
        ) {
    ...
    public User findById(Integer id) throws DaoException {
        try (
            ...
            if (resultSet.next()) {
                ...
                String contactInfo = resultSet.getString(6);
                String password = resultSet.getString(7);
                return new User(userId, name, userRole, clientInn, country,
    ...
    public void delete(User user) throws DaoException {
        try (Connection connection = ControllerFactory.connectionPoolImpl.get();
            ...
            if (user.getUserRole() == UserRole.ADMIN) {
                logger.warn("ATTENTION. DELETED ADMIN " + user.toStringShort() +
                        ", userId=" + user.getId());
            }
        } catch (SQLException e) {
            logger.error("Failed to delete user , Id = " + user.getId() + e);
            throw new DaoException("Failed to delete user by Id. " + user.getId() + e);
        }
    }
